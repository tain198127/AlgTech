    ...
    public void allInOneTest() throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
        ...
        for(Class clsItem:cls) {
            try {
                ...
                retVal = (boolean) method.invoke(clazz, UUID.randomUUID().toString());
                if(!retVal){
                    System.err.println(MessageFormat.format("{0} 执行结果出错",clsItem.getTypeName()));
//                    Assert.fail(clsItem.getTypeName()+"执行结果出错");
                }
