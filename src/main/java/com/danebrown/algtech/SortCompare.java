...
public class SortCompare {
    ...
    public static void main(String[] args) {
        ...
        boolean compareResult = sortCompare.compareWrap(
                () -> sortCompare.insertSort(),
                "insertSort",
                () -> sortCompare.standardSort(),
                false);
    ...
    public boolean compare(int[] forTest, int[] standard) {
        ...
        for (int i = 0; i < length && result; i++) {
            ...
            if (!result) {
                log.error("第{}位数据不一致，原始数据:{};测试数据:{}", i, standard[i], forTest[i]);
            }
        }
        return result;
    }
     ...
     * @return
     */
    public boolean compareWrap(Supplier<int[]> forTest, String testMethodName, Supplier<int[]> standard, boolean isCompare) {
